...
import { getAllEmployers } from '../actions/EmployerAction'
import { fetchJobsWithPagination } from '../actions/JobActions'
import { allEmployersFail } from '../slices/EmployerSlice'
...
export const Jobs = () => {
  ...
  const { allJobs, loading: jobsLoading } = useSelector(state => state.job);
  const { categories, loading: categoryLoading } = useSelector(state => state.category);
  const { allEmployers, loading: employerLoading } = useSelector(state => state.employer);
  const { jobPerPage, loading: jobPerPageLoading } = useSelector(state => state.job);
  const { totalPage, loading: totalPageLoading } = useSelector(state => state.job);
  ...
  useEffect(() => {
    if (allEmployers.length > 0) {
      const employerNames = allEmployers.map(employer => employer.name);
      setEmployerData(employerNames);
    }
  }, [allEmployers]);
  ...
  const leftFilter = (jobsList) => {
    ...
    console.log(category, wage);
    const leftFilArr = jobsList.filter((item) => (
      item.jobCategory.toLowerCase() === category.toLowerCase() && parseInt(item.wage) >= wage
    ))
    setJobs(leftFilArr)
  }
  ...
  const rightFilter = (jobsList) => {
    ...
    const rightFilArr = jobsList.filter((item) => (
      item.employerName.toLowerCase() === employer.toLowerCase()
    ))
    setJobs(rightFilArr)
  ...
  //     },
  //   };
  //   dispatch(fetchJobsWithPagination(config));
  // }, [])
          ...
          <div className='flex-col flex justify-center items-center w-full '>
            ...
            <div className='py-3 pt-4 w-full flex justify-center items-center'>
              <div className='flex  justify-center w-full  items-center  '>
                <div className='bg-gray-950 flex sm:w-2/5 w-4/5'>
            ...
            <div className=' flex flex-col pt-1 justify-between sm:flex-row w-full '>
              ...
              {!isMobile && <div className='filter1  flex  flex-col'>
                <div className='flex justify-start  flex-col filter  '>
                  ...
                  <ul className='flex pt-3 flex-col gap-3 max-h-40 overflow-y-auto'>
                  ...
                  <div className='pt-5'>
                    <p className='text-xl pb-3 underline underline-offset-4'>Thu nhập</p>
                ...
                <div className='flex  flex-col sm:overflow-y-auto  sm:max-h-[30em] gap-4'>

                  {
                    ...
                    jobs && displayedData
                      .filter(job => job.id)
                      .sort((a, b) => {
                        const dateA = new Date(a.recruitedDate);
                        const dateB = new Date(b.recruitedDate);
                        return dateB - dateA;
                      }).map((job) => (
                        <JobCard onClick={() => {
                          dispatch(getSingleJob(job.id))
                        }}
                          key={job.id}
                          job={job} />
                    ...
                    <div className='flex  justify-between pt-10 px-6'>
                      ...
                      <div className='filter1  sm:hidden flex flex-col'>
                        <div className='flex justify-start  flex-col filter  '>
                          ...
                          <ul className='flex pt-3 text-sm flex-col gap-3 max-h-40 overflow-y-auto'>
                      ...
                      <div className='filter2  sm:hidden flex flex-col ml-16'>
                        <div className='flex justify-end  flex-col filter  '>
                          ...
                          <div className='pt-3 flex flex-col justify-end text-right gap-3 max-h-40 overflow-y-auto'>
                            {
                              employerData.map(e => (
                                <div onClick={() => setEmployer(e)} className={`${employer === e ? "text-yellow-600" : ""} cursor-pointer text-sm hover:text-yellow-600`} >{e}</div>
                              ))
                            }
                          </div>
              ...
              {!isMobile && <div className='filter2 flex flex-col ml-16'>
                <div className='flex justify-end  flex-col filter  '>
                  ...
                  <div className='pt-3 flex flex-col justify-end text-right gap-3 max-h-40 overflow-y-auto'>
                    {
                      employerData.map(e => (
                        <div onClick={() => setEmployer(e)} className={`${employer === e ? "text-yellow-600" : ""} cursor-pointer hover:text-yellow-600`} >{e}</div>
                      ))
                    }
                  </div>
